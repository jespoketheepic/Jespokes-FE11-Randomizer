...
randClassesGo (p, e) classes items infiniteInfiniteRNGs disp@DisposFile{entries=entries} = disp{entries=map (\(entry, rngs) -> if (fst (chrID entry) < 0x3B && p) || (fst (chrID entry) > 0x3A && e) then randClass classes items rngs entry else entry) (zip entries infiniteInfiniteRNGs)}

randClass :: [Class] -> [Item] -> [StdGen] -> DisposEntry -> DisposEntry
randClass classes items rngs disp@DisposEntry{classID=classID, inventory=inventory} = disp{classID=randomClassOfSamePromo classes (head rngs) classID
  , inventory=matchWeaponsToClass inventory items (tail rngs) (getClassByClassNr classes (randomClassOfSamePromo classes (head rngs) classID))}

randomClassOfSamePromo :: [Class] -> StdGen -> Word8 -> Word8
randomClassOfSamePromo classes rng classID = let
  promoLv = promo (getClassByClassNr classes classID)
  clsOptions = filter (\cls -> valid cls && promoLv == promo cls) classes
  in classNr (randomListElement clsOptions rng)

getClassByClassNr :: [Class] -> Word8 -> Class
getClassByClassNr classes classID = head (filter (\cls -> classID == classNr cls) classes)

getItemByItemNr :: [Item] -> Word8 -> Item
getItemByItemNr items itemID = head (filter (\item -> itemID == itemNr item) items)
...
randReclass reclass db@DatabaseStruct{classes=classes} randGs = if reclass /= NormalReclass
then db{classes= randReclassGo reclass classes randGs}
else db

randReclassGo :: Reclass -> [Class] -> [StdGen] -> [Class]
randReclassGo NoReclass classes _ = map (\cls -> cls{reclassSet=0}) classes
randReclassGo RandomReclassSets classes randGs = map (\(cls, rng) -> randReclassSets rng cls) (zip classes randGs)
...
randReclassSets :: StdGen -> Class -> Class
randReclassSets randG cls@Class{reclassSet=reclass}
  | reclass == 0 = cls{reclassSet=0}
  | reclass < 4 = cls{reclassSet= randomR_Word8 (1, 3) randG}
  | otherwise = cls{reclassSet= randomR_Word8 (4, 6) randG}
...
prfDistribution2 characters rngg = let
  swordCands = filter (\chr -> case chrClass chr of {Nothing -> False; Just cls -> chrNr chr < 0x3B && head (wTypes cls) /= 0}) characters
  lanceCands = filter (\chr -> case chrClass chr of {Nothing -> False; Just cls -> chrNr chr < 0x3B && (wTypes cls !! 1) /= 0}) characters
  tomeCands = filter (\chr -> case chrClass chr of {Nothing -> False; Just cls -> chrNr chr < 0x3B && (wTypes cls !! 4) /= 0}) characters
  staffCands = filter (\chr -> case chrClass chr of {Nothing -> False; Just cls -> chrNr chr < 0x3B && (wTypes cls !! 5) /= 0}) characters
  stoneCands = filter (\chr -> case chrClass chr of {Nothing -> False; Just cls -> chrNr chr < 0x3B && (classNr cls == 0x27 || classNr cls == 0x28)}) characters
